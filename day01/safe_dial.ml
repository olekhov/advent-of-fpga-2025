...
(* Expose Hardcaml's deriver runtime under the name expected by [@@deriving hardcaml]. *)
...
let create (_scope : Scope.t) (i : _ I.t) : _ O.t =
  ...
  let pos_spec = Reg_spec.create ~clock:i.clk () in

  let pos = reg_fb
    pos_spec
    ~width:7
    ~f:(fun prev ->
      let next = mux2 i.step (step_pos prev i.dir) prev in
      mux2 i.rst pos_init next
    )
  in
